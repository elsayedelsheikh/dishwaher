...
class UIBuilder:
    ...
    def _setup_scene(self):
        ...
        ## Environment
        create_new_stage()
        add_reference_to_stage(get_assets_root_path() + "/Isaac/Environments/Grid/default_environment.usd", "/background")
        
        ## Assets
        ASSETS_USER_PATH = "/home/sayed/Desktop/DishWasher/github/dishwasher/Assets/"
        ...
        path_to_robot_usd = ASSETS_USER_PATH + "ur10e.usd"
        add_reference_to_stage(path_to_robot_usd, robot_prim_path)

        ## Load Plate
        plate_prim_path = "/theplate"
        path_to_plate_usd = ASSETS_USER_PATH + "plate.usd"
        add_reference_to_stage(path_to_plate_usd, plate_prim_path)

        ## Load Bin
        bin_prim_path = "/thebin"
        path_to_bin_usd = ASSETS_USER_PATH + "bin_fullmodel.usd"
        add_reference_to_stage(path_to_bin_usd, bin_prim_path)

        # self._articulation = Articulation(robot_prim_path + "/ur_10")

        # Add user-loaded objects to the World
        world = World.instance()
        world.scene.add(XFormPrim(name="manipulator", prim_path= robot_prim_path, position=[0.0, 0.0, 0.52]))   
        world.scene.add(XFormPrim(name="packing_bin", prim_path= bin_prim_path,   position=[0.5, -0.5, 0.0]))
        world.scene.add(XFormPrim(name="plate_01",    prim_path= plate_prim_path, position=[0.5, 0.5, 0.01]))
